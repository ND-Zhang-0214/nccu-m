...
- **開放需求與申請(教授/單位 → 學生,廣播式)**:2026-08 依白皮書 §2.1「事由×發起方」模型重構,
  五類改為 TA(課程助教)/RA(研究助理)/LAB(實驗室成員招募)/DEPT(系辦短期)/EXT(校外計畫指導),
  **產學跨域(IND)已依白皮書 §1.3 排除範圍移除**。申請狀態機 `pending → interview_invited → interviewed
  → accepted/rejected`,同一需求不可重複申請(資料庫層 UNIQUE 約束)。教授可於 `/postings/new` 自行發布。
- **學生發起請求(學生 → 教授,結構化,白皮書 §2.3.1/§2.9/§2.10)**:推薦信(REC)/大專生計畫(UR)/
  加入實驗室(LAB_JOIN)/校外計畫指導掛名(EXT_ENDORSE)四類,教授於儀表板「可受理的學生請求」逐項開關、
  填寫條件與名額說明;學生僅能在教授開放時看到對應請求連結並填表送出(未開放時直接擋下,無法繞過網址存取)。
  請求狀態機 `pending/wants_to_talk → accepted/declined`,REC 額外多一段 `writing → sent/declined_after_accept`
  中介狀態(對應「教授接受後仍在撰寫,尚未真正送出推薦信」的實際流程)。學生可於 `/me/requests` 追蹤進度。
  第五類「學生合作專區引導」(COLLAB_GUIDE)目前僅有教授端開關,尚未有對應的學生端提交表單(見下方已知範圍界定)。
...
## 已知範圍界定(非未實作,是誠實標註的簡化)

站內訊息、聯絡方式加密揭露、雙人核可、檔案上傳、帳號生命週期自動化、面試時段+ics、教授群組、AI輔助、學期報告 dashboard、白皮書 §2.3.1/§2.9/§2.10 學生發起請求流程等功能**皆已完成並經測試**(詳見 `PROJECT_STATE.md`、`SECURITY_REVIEW.md`、`FUNCTIONAL_TEST_CHECKLIST.md`)。

- **學生合作專區引導(COLLAB_GUIDE)**:教授端已有開關與條件說明欄位,但白皮書對應的「學生合作專區」模組本身尚未實作,因此這一類目前刻意不提供學生端送出表單(避免開了收件開關卻無法真正處理),儀表板上會明確提示教授這一項「尚無對應請求流程」。
- **CSP 安全標頭曾有一個會讓全站互動完全失效的 bug,本輪已修正並改用真實瀏覽器驗證**,不再只用 `curl -I` 檢查標頭存在(這正是舊版 `FUNCTIONAL_TEST_CHECKLIST.md` 誠實標註「建議正式上線前再過一輪人工瀏覽器 QA」的項目之一,而這輪补測就真的抓到問題)。細節與修法見 `SECURITY_REVIEW.md`。

正式上線前仍需處理的項目(非功能缺失,是「開發環境替代方案需換成正式系統」):畢業偵測與批次到期處理目前為管理員手動觸發(需接真實學籍系統/排程器)、需設定獨立 `ENCRYPTION_KEY`、需接真實寄信服務、需換 PostgreSQL、學生合作專區模組本身待開發(見上方 COLLAB_GUIDE 說明)。完整清單見 `SECURITY_REVIEW.md` 第五節。
