...
站內訊息、聯絡方式加密揭露、雙人核可、檔案上傳、帳號生命週期自動化、面試時段+ics、教授群組、AI輔助、學期報告 dashboard 等功能**皆已完成並經測試**(詳見 `PROJECT_STATE.md`、`SECURITY_REVIEW.md`、`FUNCTIONAL_TEST_CHECKLIST.md`)。

正式上線前仍需處理的項目(非功能缺失,是「開發環境替代方案需換成正式系統」):畢業偵測與批次到期處理目前為管理員手動觸發(需接真實學籍系統/排程器)、需設定獨立 `ENCRYPTION_KEY`、需接真實寄信服務、需換 PostgreSQL。完整清單見 `SECURITY_REVIEW.md` 第五節。
